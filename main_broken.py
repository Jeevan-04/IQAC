...
def add_school_page(inst_id: str):
    ...
    *:focus {
    ...
    @media (max-width: 768px) {
        ...
        .main-content {
        /* CSS content here - commented out to fix syntax */
        }
        */
        
# Functions are already defined above - removed duplicate definitions
...
def add_program_page(inst_id: str):
    ...
    with ui.row().style('position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: 1000; align-items: center; justify-content: center; backdrop-filter: blur(6px); background: rgba(0,0,0,0.25);'):
        with ui.card().style(f'align-items: center; padding: 2.5rem 2.5rem 2rem 2.5rem; background: #fff; border-radius: 18px; box-shadow: 0 4px 32px rgba(0,0,0,0.18); min-width: 400px;'):
            ui.label('Add Program').style(f'font-size: 1.7rem; font-weight: bold; color: {main_color}; margin-bottom: 2rem;')
            ...
            # Show school information if school_id is provided
            if school_id:
                try:
                    school_doc = schools_col.find_one({'_id': ObjectId(school_id)})
                    school_name = school_doc.get('name', 'Unknown') if school_doc else 'Unknown'
                    ui.label(f'School: {school_name}').style(f'font-size: 0.9rem; color: {main_color}; margin-bottom: 1rem; font-weight: bold;')
                except:
                    ui.label('⚠️ Invalid school ID').style('font-size: 0.9rem; color: #c00; margin-bottom: 1rem; font-weight: bold;')
                    school_id = None
            else:
                ui.label('⚠️ No school selected').style('font-size: 0.9rem; color: #c00; margin-bottom: 1rem; font-weight: bold;')
                ui.label('Please go back to the hierarchy page and select a school first.').style('font-size: 0.8rem; color: #666; margin-bottom: 1rem;')
            
            program_name = ui.input('Program Name', placeholder='e.g., Computer Science, MBA').style('width: 350px; margin-bottom: 1rem;')
            
            # Program Type Selection
            program_type = ui.select(
                options=['UG (Under Graduate)', 'PG (Post Graduate)', 'Diploma', 'Certificate'],
                value='UG (Under Graduate)',
                label='Program Type'
            ...
            preview_code = ui.label('Program Code: (will be generated)').style('font-size: 0.9rem; color: #666; margin-bottom: 1rem; font-style: italic;')
...
def institution_admin_sidebar(inst_id, main_content_func):
    print(f"DEBUG: institution_admin_sidebar called with ID: {inst_id}")
    try:
        ...
        from bson import ObjectId
        print(f"DEBUG: Validating ObjectId: {inst_id}")
        
        # Validate ObjectId format
        if not ObjectId.is_valid(inst_id):
            print(f"DEBUG: Invalid ObjectId format: {inst_id}")
            ui.label(f'Invalid institution ID format: {inst_id}').style('color: red; font-size: 1.2rem; margin: 2rem;')
            ui.button('Go to Login', on_click=lambda: ui.navigate.to('/')).style('margin: 1rem;')
            return
            
        print(f"DEBUG: Looking up institution in database")
        inst = institutions_col.find_one({'_id': ObjectId(inst_id)})
        if not inst:
            print(f"DEBUG: Institution not found in database for ID: {inst_id}")
            ui.label(f'Institution not found for ID: {inst_id}').style('color: red; font-size: 1.2rem; margin: 2rem;')
            ui.button('Go to Login', on_click=lambda: ui.navigate.to('/')).style('margin: 1rem;')
            return
            
        print(f"DEBUG: Institution found: {inst.get('name', 'Unknown')}")
    except Exception as e:
        print(f"DEBUG: Exception in sidebar: {str(e)}")
        ui.label(f'Database error: {str(e)}').style('color: red; font-size: 1.2rem; margin: 2rem;')
        ui.button('Go to Login', on_click=lambda: ui.navigate.to('/')).style('margin: 1rem;')
        return
    main_color = inst.get('theme_color', '#1976d2')
...
@ui.page('/test')
def test_page():
    ui.label('Test page works!').style('font-size: 2rem; color: green; margin: 2rem;')
    ui.button('Go to Login', on_click=lambda: ui.navigate.to('/')).style('margin: 1rem;')

# --- Institution Admin Dashboard ---
@ui.page('/institution_admin/{inst_id}')
def institution_admin_dashboard(inst_id: str):
    print(f"DEBUG: Accessing institution dashboard for ID: {inst_id}")
    try:
        def content(inst, main_color):
        ...
        print(f"DEBUG: About to call institution_admin_sidebar with ID: {inst_id}")
        institution_admin_sidebar(inst_id, content)
        print(f"DEBUG: Successfully called institution_admin_sidebar")
    except Exception as e:
        print(f"DEBUG: Exception in dashboard: {str(e)}")
        ui.label(f'Error loading dashboard: {str(e)}').style('color: red; font-size: 1.2rem; margin: 2rem;')
        ui.label(f'Institution ID: {inst_id}').style('color: #666; margin: 1rem;')
        ui.button('Go to Login', on_click=lambda: ui.navigate.to('/')).style('margin: 1rem;')
...
def institution_admin_hierarchy(inst_id: str):
    def content(inst, main_color):
        ...
        with ui.column().style('width: 100%; align-items: flex-start;'):
            # --- Minimal academic year dropdown (same as extended profiles) ---
            unlocked_years = list(academic_years_col.find({'institution_id': inst_id, 'is_locked': False}))
            year_options = [y['name'] for y in unlocked_years]
            name_to_id = {y['name']: str(y['_id']) for y in unlocked_years}
    ...
    # Drag and drop JavaScript functionality temporarily commented out to fix syntax
    # ui.add_head_html("""
    # <script>
    ...
    institution_admin_sidebar(inst_id, content)
    # Temporarily commenting out JavaScript drag and drop functionality to fix syntax errors
    # All JavaScript code from lines 1084-1172 has been commented out
...
def institution_admin_extended_profiles(inst_id: str):
    def content(inst, main_color):
        ...
        with ui.row().style('width: 100%; align-items: center; justify-content: space-between;'):
            ui.label('Extended Profiles').style(f'font-size: 1.5rem; font-weight: bold; color: {main_color}; margin-top: 2rem;')
    ...
    def content(inst, main_color):
        ...
        with ui.row().style('width: 100%; align-items: center; justify-content: flex-end; margin-bottom: 1rem;'):
            ...
            if year_options:
                ...
                ui.label('No academic years created yet.').style('color: #c00; font-weight: bold; margin-left: 0.2rem; font-size: 1rem;')
        with ui.row().style('width: 100%; align-items: center; justify-content: space-between;'):
            ui.label('Criterias').style(f'font-size: 1.5rem; font-weight: bold; color: {main_color}; margin-top: 2rem;')
            def open_create_dialog():
                if not selected_year_id:
                    ui.notify('Select an academic year first', color='negative')
                    return
                ...
                with ui.element('table').classes('data-grid').style('width: 100%; border-collapse: collapse;'):
                    ...
                    with ui.element('tbody'):
                        ...
                        def render_enhanced_field_cell(field, row_data, row_idx, programs, main_color):
                            ...
                            cell_style = 'padding: 0.25rem; border: 1px solid #dee2e6; position: relative;'
                            
                            if field.get('auto_fill') and row_data.get('_program_id'):
                                cell_style += ' background: #e8f5e9;'  # Light green for auto-filled
                            
                            if field.get('required') and not str(cell_value).strip():
                                cell_style += ' border-left: 4px solid #f44336;'  # Red border for required empty fields
                            
                            with ui.element('td').style(cell_style):
                                if field.get('auto_fill') and row_data.get('_program_id'):
                                    # Read-only auto-filled field with lock icon
                                    ui.label(str(cell_value) if cell_value else '').style('font-weight: 500; color: #2e7d32;')
                                else:
                                    # Editable field
                                    render_editable_field(field, row_data, row_idx, field_name, cell_value)
    ...
    def do_login():
        ...
        if user.get('must_change_password', False):
            ...
            if user.get('role', '') == 'Institution Admin' and user.get('institution_id'):
                # Temporarily redirect to test page instead of institution admin
                ui.navigate.to('/test')
...
def dashboard():
    ...
    with ui.element('div').style('min-height: 100vh; display: flex;'):
        # Beautiful Sidebar
        with ui.element('div').classes('beautiful-sidebar'):
            # Sidebar Header
            with ui.element('div').classes('sidebar-header'):
                ui.icon('dashboard', size='2rem').style('margin-bottom: 0.5rem;')
                ui.label('IQAC Portal').style('font-size: 1.2rem; font-weight: 700; letter-spacing: 0.025em;')
                ui.label('Management Dashboard').style('font-size: 0.8rem; opacity: 0.9; margin-top: 0.25rem;')
            
            # Navigation Items
            with ui.column().style('padding: 1.5rem 0; flex: 1;'):
                ui.link('🏠 Dashboard', '/dashboard').classes('sidebar-nav-item active')
                ui.link('🏢 Institutions', '/dashboard').classes('sidebar-nav-item')
                ui.link('📊 Analytics', '/dashboard').classes('sidebar-nav-item')
                ui.link('👥 Users', '/dashboard').classes('sidebar-nav-item')
                ui.link('⚙️ Settings', '/dashboard').classes('sidebar-nav-item')
            
            # User Profile Section
            with ui.column().style('padding: 1.5rem; border-top: 1px solid rgba(255, 255, 255, 0.3); margin-top: auto;'):
                initials = (first_name[:1] + last_name[:1]).upper() if first_name or last_name else email[:2].upper()
                
                with ui.row().style('align-items: center; gap: 1rem; margin-bottom: 1rem;'):
                    ui.label(initials).style('width: 48px; height: 48px; border-radius: 50%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; font-weight: 700; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.3);')
                    
                    with ui.column().style('flex: 1;'):
                        ui.label(f'{first_name} {last_name}' if first_name or last_name else 'User').style('font-weight: 600; font-size: 0.9rem; color: #2c3e50;')
                        ui.label(email).style('font-size: 0.75rem; color: rgba(44, 62, 80, 0.7); margin-top: 0.25rem;')
                
                with ui.row().style('gap: 0.5rem;'):
                    ui.button('👤', on_click=lambda: ui.navigate.to('/profile')).style('flex: 1; padding: 0.5rem; background: rgba(102, 126, 234, 0.1); color: #667eea; border: 1px solid rgba(102, 126, 234, 0.2); border-radius: 6px;')
                    ui.button('🚪', on_click=lambda: ui.navigate.to('/')).style('flex: 1; padding: 0.5rem; background: rgba(244, 67, 54, 0.1); color: #f44336; border: 1px solid rgba(244, 67, 54, 0.2); border-radius: 6px;')
        
        # Main Content Area
        with ui.element('div').classes('main-content'):
            # Page Header
            with ui.element('div').classes('page-header animate-fade-in-up'):
                ui.label('🎯 Institution Management').classes('page-title')
                ui.label('Manage and monitor all educational institutions').classes('page-subtitle')
            
            # Quick Stats
            with ui.row().style('gap: 1.5rem; margin-bottom: 2rem; width: 100%;').classes('animate-fade-in-up'):
                # Total Institutions
                with ui.element('div').classes('stats-card').style('flex: 1;'):
                    ui.label(str(len(institutions))).classes('stats-number')
                    ui.label('Total Institutions').classes('stats-label')
                
                # Active Users (placeholder)
                with ui.element('div').classes('stats-card').style('flex: 1;'):
                    total_users = users_col.count_documents({})
                    ui.label(str(total_users)).classes('stats-number')
                    ui.label('Total Users').classes('stats-label')
                
                # This Month Activity (placeholder)
                with ui.element('div').classes('stats-card').style('flex: 1;'):
                    ui.label('42').classes('stats-number')
                    ui.label('Active This Month').classes('stats-label')
            
            # Action Section
            with ui.row().style('justify-content: center; margin-bottom: 3rem;').classes('animate-slide-in-right'):
                ui.button('🏗️ Create New Institution', on_click=lambda: ui.navigate.to('/create_institution')).classes('btn-beautiful').style('padding: 1rem 2rem; font-size: 1.1rem; font-weight: 600;')
            
            # Institutions Grid
            if institutions:
                with ui.column().style('width: 100%;').classes('animate-fade-in-up'):
                    ui.label('🏢 Your Institutions').style('font-size: 1.5rem; font-weight: 700; color: #2c3e50; margin-bottom: 1.5rem; text-align: center;')
                    
                    with ui.element('div').style('display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 1.5rem; width: 100%;'):
                        for inst in institutions:
                            ...
                            def create_institution_card(institution=inst, color=main_color):
                                ...
                                with ui.card().classes('beautiful-card').style(f'cursor: pointer; position: relative; overflow: hidden;').on('click', go_to_institution):
                                    # Gradient accent bar
                                    ui.element('div').style(f'position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, {color}, {color}88);')
                                    
                                    with ui.column().style('align-items: center; text-align: center; padding: 1rem;'):
                                        # Logo
                                        logo_url = institution.get('logo') or f'https://ui-avatars.com/api/?name={institution.get("name", "Institution")}&background={color[1:]}&color=fff&size=80'
                                        ui.image(logo_url).style('width: 80px; height: 80px; border-radius: 50%; object-fit: cover; margin-bottom: 1rem; box-shadow: 0 4px 15px rgba(0,0,0,0.1);')
                                        
                                        # Institution Info
                                        ui.label(institution.get('name', 'Institution')).style('font-size: 1.2rem; font-weight: 700; color: #2c3e50; margin-bottom: 0.5rem;')
                                        
                                        if institution.get('location'):
                                            ui.label(f"📍 {institution.get('location')}").style('font-size: 0.9rem; color: rgba(44, 62, 80, 0.7); margin-bottom: 0.5rem;')
                                        
                                        if institution.get('type'):
                                            ui.label(f"🎓 {institution.get('type')}").style('font-size: 0.85rem; color: rgba(44, 62, 80, 0.6); margin-bottom: 1rem;')
                                        
                                        # Action Button
                                        ui.button('🚀 Manage Institution', on_click=go_to_institution).classes('btn-beautiful').style(f'background: linear-gradient(135deg, {color}, {color}88); width: 100%; margin-top: 1rem;')
                            
                            create_institution_card()
            else:
                # Empty State
                with ui.column().style('align-items: center; text-align: center; padding: 3rem; margin-top: 2rem;').classes('animate-fade-in-up'):
                    ui.icon('school', size='6rem').style('color: rgba(102, 126, 234, 0.3); margin-bottom: 1.5rem;')
                    ui.label('No Institutions Yet').style('font-size: 1.5rem; font-weight: 700; color: rgba(44, 62, 80, 0.7); margin-bottom: 0.5rem;')
                    ui.label('Create your first institution to get started with quality assurance management.').style('color: rgba(44, 62, 80, 0.6); margin-bottom: 2rem; max-width: 400px;')
                    ui.button('🏗️ Create Your First Institution', on_click=lambda: ui.navigate.to('/create_institution')).classes('btn-beautiful').style('padding: 1rem 2rem; font-size: 1.1rem;')
...
def create_institution_page():
    ...
    with ui.element('div').style('min-height: 100vh; background: var(--gradient-primary); display: flex; align-items: center; justify-content: center; padding: 2rem;'):
        with ui.element('div').classes('beautiful-card').style('max-width: 600px; width: 100%; padding: 0;').classes('animate-fade-in-up'):
            # Header
            with ui.element('div').style('background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: white; padding: 2rem; text-align: center; border-radius: 12px 12px 0 0;'):
                ui.icon('school', size='3rem').style('margin-bottom: 1rem; color: white; filter: drop-shadow(0 2px 8px rgba(0,0,0,0.2));')
                ui.label('Create New Institution').style('font-size: 1.8rem; font-weight: 700; margin-bottom: 0.5rem;')
                ui.label('Set up your educational institution for quality assurance management').style('font-size: 1rem; opacity: 0.9;')
            ...
            with ui.element('div').style('padding: 2.5rem;'):
                with ui.column().style('gap: 1.5rem; width: 100%;'):
                    ...
                    with ui.element('div').classes('form-group'):
                        ui.label('🏢 Institution Name').classes('form-label')
                        name_input = ui.input(placeholder='Enter institution name').classes('form-input')
                    
                    # Location
                    with ui.element('div').classes('form-group'):
                        ui.label('📍 Location').classes('form-label')
                        location_input = ui.input(placeholder='City, State/Province, Country').classes('form-input')
                    
                    # Type
                    with ui.element('div').classes('form-group'):
                        ui.label('🎓 Institution Type').classes('form-label')
                        type_select = ui.select(['University', 'College', 'School', 'Research Institute', 'Technical Institute', 'Other'], 
                                               placeholder='Select institution type').classes('form-input')
                    
                    # Establishment Year
                    with ui.element('div').classes('form-group'):
                        ui.label('📅 Establishment Year').classes('form-label')
                        year_input = ui.number(placeholder='e.g., 1985', format='%.0f').classes('form-input')
                    
                    # Website
                    with ui.element('div').classes('form-group'):
                        ui.label('🌐 Website (Optional)').classes('form-label')
                        website_input = ui.input(placeholder='https://www.example.edu').classes('form-input')
                    
                    # Theme Color
                    with ui.element('div').classes('form-group'):
                        ui.label('🎨 Theme Color').classes('form-label')
                        with ui.row().style('align-items: center; gap: 1rem;'):
                            color_input = ui.color_input(label='', value='#667eea').classes('form-input').style('flex: 1;')
                            ui.label('Choose a primary color for your institution\'s branding').style('font-size: 0.85rem; color: rgba(44, 62, 80, 0.7);')
                    
                    # Logo URL
                    with ui.element('div').classes('form-group'):
                        ui.label('🖼️ Logo URL (Optional)').classes('form-label')
                        logo_input = ui.input(placeholder='https://example.com/logo.png').classes('form-input')
                        ui.label('We\'ll generate a default logo if not provided').style('font-size: 0.85rem; color: rgba(44, 62, 80, 0.7); margin-top: 0.5rem;')
                    
                    # Description
                    with ui.element('div').classes('form-group'):
                        ui.label('📝 Description (Optional)').classes('form-label')
                        description_input = ui.textarea(placeholder='Brief description of your institution...').classes('form-input').style('min-height: 100px; resize: vertical;')
                    
                    # Action Buttons
                    with ui.row().style('gap: 1rem; justify-content: center; margin-top: 2rem;'):
                        ui.button('← Back to Dashboard', on_click=lambda: ui.navigate.to('/dashboard')).classes('btn-secondary').style('padding: 0.75rem 1.5rem;')
    ...
    with ui.element('div').style('min-height: 100vh; display: flex;'):
        ...
        with ui.element('div').classes('beautiful-sidebar').style(f'background: linear-gradient(180deg, {main_color}15, {main_color}05);'):
            ...
            with ui.element('div').classes('sidebar-header').style(f'border-bottom: 1px solid {main_color}30;'):
                logo_url = institution.get('logo') or f'https://ui-avatars.com/api/?name={institution_name}&background={main_color[1:]}&color=fff&size=60'
            ...
            # Navigation Items
            with ui.column().style('padding: 1.5rem 0; flex: 1;'):
                ui.link('🏠 Dashboard', f'/institution/{inst_id}').classes('sidebar-nav-item active')
                ui.link('👥 Users', f'/institution/{inst_id}/users').classes('sidebar-nav-item')
        ...
        with ui.element('div').classes('main-content'):
            ...
            with ui.element('div').classes('page-header animate-fade-in-up').style(f'border-bottom: 1px solid {main_color}20;'):
                ui.label(f'🎯 {institution_name}').classes('page-title').style(f'color: {main_color};')
                ui.label('Institution Quality Assurance & Accreditation Center').classes('page-subtitle')
                
                # Institution Info Pills
                with ui.row().style('gap: 1rem; margin-top: 1rem; flex-wrap: wrap;'):
                    if institution.get('location'):
                        with ui.element('div').style(f'background: {main_color}15; color: {main_color}; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.85rem; font-weight: 500;'):
                            ui.label(f"📍 {institution.get('location')}")
                    if institution.get('type'):
                        with ui.element('div').style(f'background: {main_color}15; color: {main_color}; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.85rem; font-weight: 500;'):
                            ui.label(f"🎓 {institution.get('type')}")
                    if institution.get('establishment_year'):
                        with ui.element('div').style(f'background: {main_color}15; color: {main_color}; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.85rem; font-weight: 500;'):
                            ui.label(f"📅 Est. {institution.get('establishment_year')}")
            
            # Quick Stats
            with ui.row().style('gap: 1.5rem; margin-bottom: 2rem; width: 100%;').classes('animate-fade-in-up'):
                # Total Users
                total_users = users_col.count_documents({'institution_id': inst_id})
                with ui.element('div').classes('stats-card').style('flex: 1;'):
                    ui.label(str(total_users)).classes('stats-number').style(f'color: {main_color};')
                    ui.label('Total Users').classes('stats-label')
                
                # Total Criteria
                total_criteria = criteria_col.count_documents({'institution_id': inst_id}) if 'criteria_col' in globals() else 0
                with ui.element('div').classes('stats-card').style('flex: 1;'):
                    ui.label(str(total_criteria)).classes('stats-number').style(f'color: {main_color};')
                    ui.label('Quality Criteria').classes('stats-label')
                
                # Total Programs
                total_programs = programs_col.count_documents({'institution_id': inst_id}) if 'programs_col' in globals() else 0
                with ui.element('div').classes('stats-card').style('flex: 1;'):
                    ui.label(str(total_programs)).classes('stats-number').style(f'color: {main_color};')
                    ui.label('Academic Programs').classes('stats-label')
            
            # Action Cards
            with ui.element('div').style('display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem; margin-bottom: 2rem;').classes('animate-slide-in-right'):
                # Manage Users Card
                with ui.element('div').classes('beautiful-card action-card').style('cursor: pointer;').on('click', lambda: ui.navigate.to(f'/institution/{inst_id}/users')):
                    ui.icon('group', size='2.5rem').style(f'color: {main_color}; margin-bottom: 1rem;')
                    ui.label('👥 Manage Users').style('font-size: 1.3rem; font-weight: 700; color: #2c3e50; margin-bottom: 0.5rem;')
                    ui.label('Add, edit, and manage institution users and their roles').style('color: rgba(44, 62, 80, 0.7); margin-bottom: 1.5rem;')
                    ui.button('Manage Users →', on_click=lambda: ui.navigate.to(f'/institution/{inst_id}/users')).classes('btn-beautiful').style(f'background: linear-gradient(135deg, {main_color}, {main_color}88); width: 100%;')
                
                # Quality Criteria Card
                with ui.element('div').classes('beautiful-card action-card').style('cursor: pointer;').on('click', lambda: ui.navigate.to(f'/institution/{inst_id}/criteria')):
                    ui.icon('checklist', size='2.5rem').style(f'color: {main_color}; margin-bottom: 1rem;')
                    ui.label('📋 Quality Criteria').style('font-size: 1.3rem; font-weight: 700; color: #2c3e50; margin-bottom: 0.5rem;')
                    ui.label('Define and manage quality assurance criteria and standards').style('color: rgba(44, 62, 80, 0.7); margin-bottom: 1.5rem;')
                    ui.button('Manage Criteria →', on_click=lambda: ui.navigate.to(f'/institution/{inst_id}/criteria')).classes('btn-beautiful').style(f'background: linear-gradient(135deg, {main_color}, {main_color}88); width: 100%;')
                
                # Academic Programs Card
                with ui.element('div').classes('beautiful-card action-card').style('cursor: pointer;').on('click', lambda: ui.navigate.to(f'/institution/{inst_id}/programs')):
                    ui.icon('school', size='2.5rem').style(f'color: {main_color}; margin-bottom: 1rem;')
                    ui.label('📊 Academic Programs').style('font-size: 1.3rem; font-weight: 700; color: #2c3e50; margin-bottom: 0.5rem;')
                    ui.label('Manage academic programs and their quality assessments').style('color: rgba(44, 62, 80, 0.7); margin-bottom: 1.5rem;')
                    ui.button('Manage Programs →', on_click=lambda: ui.navigate.to(f'/institution/{inst_id}/programs')).classes('btn-beautiful').style(f'background: linear-gradient(135deg, {main_color}, {main_color}88); width: 100%;')
                
                # Reports & Analytics Card
                with ui.element('div').classes('beautiful-card action-card').style('cursor: pointer;').on('click', lambda: ui.navigate.to(f'/institution/{inst_id}/reports')):
                    ui.icon('analytics', size='2.5rem').style(f'color: {main_color}; margin-bottom: 1rem;')
                    ui.label('📈 Reports & Analytics').style('font-size: 1.3rem; font-weight: 700; color: #2c3e50; margin-bottom: 0.5rem;')
                    ui.label('View comprehensive reports and quality analytics').style('color: rgba(44, 62, 80, 0.7); margin-bottom: 1.5rem;')
                    ui.button('View Reports →', on_click=lambda: ui.navigate.to(f'/institution/{inst_id}/reports')).classes('btn-beautiful').style(f'background: linear-gradient(135deg, {main_color}, {main_color}88); width: 100%;')
            
            # Recent Activity Section
            with ui.element('div').classes('animate-fade-in-up'):
                ui.label('📊 Recent Activity').style('font-size: 1.5rem; font-weight: 700; color: #2c3e50; margin-bottom: 1.5rem;')
                
                with ui.element('div').classes('beautiful-card').style('padding: 2rem;'):
                    # For now, show placeholder content since audit_logs_col might not exist
                    with ui.column().style('align-items: center; text-align: center; padding: 2rem;'):
                        ui.icon('timeline', size='3rem').style('color: rgba(102, 126, 234, 0.3); margin-bottom: 1rem;')
                        ui.label('No Recent Activity').style('font-size: 1.1rem; font-weight: 600; color: rgba(44, 62, 80, 0.7); margin-bottom: 0.5rem;')
                        ui.label('Activity will appear here as you start using the system.').style('color: rgba(44, 62, 80, 0.6);')
